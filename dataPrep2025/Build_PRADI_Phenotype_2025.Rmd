...
```{r}
AAAD_SOCIO_DEMO <- as.data.frame(read_excel("/Users/mengnazhang/Desktop/ADSP_DataPrep_local/PRADI/Phenotype/2025/Raw/PRADI_AAAD_SOCIO_DEMO_05122025.xlsx",sheet = "Export Worksheet"))
...
## view those variables in the regeneraed DD
dfDD[dfDD$VarNames %in% mismatchChrs_1,c("VarNames","Data Type")]

## after checking the unique values of variables in the mismatchChrs_1, I believe that they all should be characters
if(length(mismatchChrs_1)>0){
  uniqueValforChrColumns(df[,mismatchChrs_1])
}
## so I will update the DD for those variables (I will change their data type in DD and switch the values from "Valid Responses" and " Valid Responses Codes" columns)
...
mismatchNums_1 <- setdiff(numcols,numColsfromDD$VarNames) ## character(0)
mismatchNums_2 <- setdiff(numColsfromDD$VarNames,numcols) ## [1] "IMPRESSION"       "AD_CATEGORY"      "AD_HX_CATEGORY"   "UNCLEAR_CATEGORY"
## ignore those variables, they have been fixed in previous step
...
## ignore these variables, they have been corrected from previous step
```
