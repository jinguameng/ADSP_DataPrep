...
```{r}
df_names <- ls()[sapply(ls(), function(x) is.data.frame(get(x)))] ## 58 datasets
```
...
cat("Before duplicates handling - wrapnum*VisNo is: ",dupFixCheck(Imaging_metadata,"wrapnum","VisNo"),"\n")
...
cat("After duplicates handling - WRAPNo*VisNo is: ",dupFixCheck(Imaging_metadata,"WRAPNo","VisNo"),"\n")
...
info(merged_data_long, "WRAPNo") #obs:8982, cols:2328, inds:1836
...
info(merged_data_all, "WRAPNo") #obs:8984, cols:2499, inds:1838 - looks like we are up 2 inds/2 obs from longitudinal and up 22 inds from cross
```
...
## Final Merge Check
```{r}
#look more into the increase to make sure it makes sense
in_cross_not_long <- anti_join(merged_data_cross, merged_data_long, by = "WRAPNo") #confirmed nothing like them in long
info(in_cross_not_long, "WRAPNo")

in_long_not_cross <- anti_join(merged_data_long, merged_data_cross, by = "WRAPNo") #confirmed nothing like them in cross
info(in_long_not_cross, "WRAPNo")

#realistically the 2+22 will probably be dropped from analyses for not having important info (age/birth/sex/diagnosis), but ok to leave in for now
```
...
```{r}
## save rds
saveRDS(merged_data_all,paste0(out_directory,"Build_WRAP.rds"))
```
