...
```{r,eval=FALSE}
## get the names of all data frames in the current environment
df_names <- ls()[sapply(ls(), function(x) is.data.frame(get(x)))]

# Initialize mergedDat with the first data frame
mergedDat <- get(df_names[1])

# Iteratively merge the rest
for (i in 2:length(df_names)) {
  mergedDat <- merge(mergedDat, get(df_names[i]), all = TRUE)
}

info(mergedDat,"wrapnum")
```
...
df_summary <- data.frame(
  Rows = sapply(df_names, function(x) nrow(get(x))),
  Columns = sapply(df_names, function(x) ncol(get(x))),
  Individuals = sapply(df_names, function(x) {
    dat <- get(x)
    length(unique(dat$wrapnum))
  })
)
