...
## if you check the img_MR_wVisNo, you will notice that some people have mutiple observations by wrapnum * VisNo
## the reason is that, when we merging with wrapnum, we grouped by wrapnum, Days_Since_Baseline_Imaging_MR to make sure each individual at each 
## imaging visit, they can be matched to a clinical visit. The next thing is we need to only to pick one imgaing obs after grouping by wrapnum * VisNo
## and the picking criteria will be exactly as how we handle the duplicates: pick the obs with minimum missingness
